...
export async function GET(_request: NextRequest) {
  ...
  const [providers, benchmark, citations, signals] = await Promise.all([
  ...
  if (providers == null && benchmark == null && citations == null && signals == null) {
    return NextResponse.json({ ready: false, reason: 'Warehouse views not available — apply migration 030 and refresh.' })
  }
