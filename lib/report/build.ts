...
export async function buildReportPdf(
  ...
  const [{ data: vs }, { data: mentions }, { data: competitors }, { data: recommendations }, { data: websiteAudit }, { data: modelRuns }, { data: promptRuns }, { data: entities }] = await Promise.all([
    ...
    supabaseAdmin.from('recommendations').select('title, description, priority, suggested_fix, expected_impact, priority_rank, impact_level, effort, confidence, evidence, data_source, benchmark').eq('audit_id', auditId).order('created_at', { ascending: true }),
  ...
  const recs = (recommendations ?? []).map((r) => ({
    ...
    data_source: r.data_source ?? null, benchmark: r.benchmark ?? null,
  }))
